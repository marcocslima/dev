...
/*   Updated: 2022/10/31 18:43:31 by mcesar-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	builtin_execute(t_data **data, t_cursors *crs)
{
	...
	crs->s = 1;
	if (((*data)->cmds[crs->i2][0] && crs->i > 0 && crs->i2 > 0 && (*data)->cmds[crs->i2 - 1][2]
		&& ft_strncmp((*data)->cmds[crs->i2 - 1][2], ">", 1)) || ((*data)->cmds[crs->i2][0]
		&& crs->i == 0))
		(*data)->tmp2 = ft_strdup((*data)->cmds[crs->i2][0]);
	while ((*data)->cmds[crs->i2] && (*data)->cmds[crs->i2][crs->s])
	{
		if ((*data)->cmds[crs->i2][crs->s] && ft_strncmp((*data)->cmds[crs->i2][crs->s], "<", 1)
			&& ft_strncmp((*data)->cmds[crs->i2][crs->s], ">", 1)
			&& ft_strncmp((*data)->cmds[crs->i2][crs->s], "|", 1)
			&& ft_strncmp((*data)->cmds[crs->i2][crs->s], ";", 1))
		{
			(*data)->tmp2 = ft_strjoin((*data)->tmp2, " ");
			(*data)->tmp2 = ft_strjoin((*data)->tmp2, (*data)->cmds[crs->i2][crs->s]);
		}
		crs->s++;
	}
